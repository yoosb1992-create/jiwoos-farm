...
function RectFields({ rect, onChange }: { rect: TileRect; onChange: (rect: TileRect) => void }) {
  return <div className="inspector-grid"><NumberField label="startX" value={rect.startX} onChange={(startX) => onChange({ ...rect, startX })} /><NumberField label="startY" value={rect.startY} onChange={(startY) => onChange({ ...rect, startY })} /><NumberField label="endX" value={rect.endX} onChange={(endX) => onChange({ ...rect, endX })} /><NumberField label="endY" value={rect.endY} onChange={(endY) => onChange({ ...rect, endY })} /></div>;
}
...
export function MapInspector({ map, maps, selection, update, renameId, remove, spawnReferences }: {
  ...
  if (selection.kind === "object") {
    ...
    return <aside className="editor-inspector"><div className="panel-title"><span>Object</span><small>{object.id}</small></div><div className="inspector-form">
      ...
      <NumberField label="depth" value={object.depth ?? 3} onChange={(depth) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.depth = depth; })} />
      <TextField label="label" value={object.label ?? ""} onChange={(label) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.label = label || undefined; })} />
      <details><summary>displaySizeOverride</summary>{object.displaySizeOverride ? <div className="inspector-grid"><NumberField label="width" value={object.displaySizeOverride.width} onChange={(width) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.displaySizeOverride!.width = width; })} /><NumberField label="height" value={object.displaySizeOverride.height} onChange={(height) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.displaySizeOverride!.height = height; })} /><button onClick={() => update((target) => { delete target.objects.find((entry) => entry.id === object.id)!.displaySizeOverride; })}>override 제거</button></div> : <button onClick={() => update((target) => { target.objects.find((entry) => entry.id === object.id)!.displaySizeOverride = { width: 64, height: 64 }; })}>override 추가</button>}</details>
      <details open><summary>collision · top-left offset</summary>{object.collision ? <><PixelRectFields rect={object.collision} onChange={(collision) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.collision = collision; })} /><button onClick={() => update((target) => { delete target.objects.find((entry) => entry.id === object.id)!.collision; })}>collision 제거</button></> : <button onClick={() => update((target) => { target.objects.find((entry) => entry.id === object.id)!.collision = { x: -16, y: -16, width: 32, height: 32 }; })}>collision 추가</button>}</details>
      <details><summary>interaction</summary>{object.interaction ? <><label><span>action</span><select value={object.interaction.action} onChange={(event) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.interaction!.action = event.target.value as "sleep" | "open_shop" | "sell"; })}><option value="sleep">sleep</option><option value="open_shop">open_shop</option><option value="sell">sell</option></select></label><RectFields rect={object.interaction.area} onChange={(area) => update((target) => { target.objects.find((entry) => entry.id === object.id)!.interaction!.area = area; })} /><button onClick={() => update((target) => { delete target.objects.find((entry) => entry.id === object.id)!.interaction; })}>interaction 제거</button></> : <button onClick={() => update((target) => { target.objects.find((entry) => entry.id === object.id)!.interaction = { action: "sell", area: { startX: 0, endX: 1, startY: 0, endY: 1 } }; })}>interaction 추가</button>}</details>
      {deleteButton}
    </div></aside>;
  }
  if (selection.kind === "spawn") {
    ...
    return <aside className="editor-inspector"><div className="panel-title"><span>Spawn</span><small>{refs ? `${refs}개 warp가 참조 중` : "참조 없음"}</small></div><div className="inspector-form">
      ...
      {refs > 0 && <p className="inspector-warning">삭제하면 참조 중인 warp가 무효가 됩니다.</p>}{deleteButton}
    </div></aside>;
  }
  if (selection.kind === "warp") {
    ...
    return <aside className="editor-inspector"><div className="panel-title"><span>Warp</span><small>{warp.id}</small></div><div className="inspector-form">
      ...
      <label><span>target spawn</span><select value={warp.targetSpawnId} onChange={(event) => update((targetMap) => { targetMap.warps.find((entry) => entry.id === warp.id)!.targetSpawnId = event.target.value; })}>{target?.spawns.map((spawn) => <option key={spawn.id}>{spawn.id}</option>)}</select></label>
      {deleteButton}
    </div></aside>;
  }
